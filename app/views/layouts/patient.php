<?php /** Patient portal layout. Expects $content, optional $page_title/$extra_head/$extra_scripts. */ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= e($page_title ?? 'Patient Portal') ?> - Yibera</title>
...
<body class="bg-gray-50">
    <div class="flex h-screen overflow-hidden">
        <?php require BASE_PATH . '/app/views/partials/patient-sidebar.php'; ?>

        <div class="flex-1 flex flex-col overflow-hidden">
            <?php require BASE_PATH . '/app/views/partials/patient-header.php'; ?>

            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-4 sm:p-6 pb-24 lg:pb-6">
                <?php if ($msg = flash('portal_success')): ?>
    ...
    <?php
    $bnActive = $_GET['page'] ?? 'portal/dashboard';
    $bnItems = [
        ['portal/dashboard',     'fa-gauge-high',          'Home'],
        ['portal/appointments',  'fa-calendar-check',      'Appts'],
        ['portal/prescriptions', 'fa-prescription',        'Meds'],
        ['portal/invoices',      'fa-file-invoice-dollar', 'Billing'],
        ['portal/profile',       'fa-user',                'Profile'],
    ];
    ?>
    ...
    <script>
        ...
        function togglePortalMenu() {
            const m = document.getElementById('portalUserMenu');
            if (m) m.classList.toggle('hidden');
        }
        document.addEventListener('click', function (e) {
            const m = document.getElementById('portalUserMenu');
            const btn = e.target.closest('button[onclick="togglePortalMenu()"]');
            if (m && !btn && !m.contains(e.target)) m.classList.add('hidden');
        });
        // Mobile off-canvas sidebar
        (function () {
            const sb = document.getElementById('portalSidebar');
            const ov = document.getElementById('portalSidebarOverlay');
            const tg = document.getElementById('portalSidebarToggle');
